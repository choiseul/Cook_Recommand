...
public class SignUpActivity extends AppCompatActivity {
    ...
    private boolean errorCk(){
        ...
        if(!matcher.matches()) {
            Snackbar
                    .make(coordinatorLayoutView, "?¥Î©î???ïÏãù???¨Î∞îÎ•¥Ï? ?äÏäµ?àÎã§.", Snackbar.LENGTH_LONG)
                    .show();
            return false;
        }else if(usernameEt.getText().toString().length() < 6) {
            Snackbar
                    .make(coordinatorLayoutView, "?ÑÏù¥?îÎ? 6?êÎ¶¨ ?¥ÏÉÅ ?ÖÎ†•?¥Ï£º?∏Ïöî.", Snackbar.LENGTH_LONG)
                    .show();
            return false;
        }else if(!passwordEt.getText().toString().equals(passwordEt2.getText().toString())){
            Snackbar
                    .make(coordinatorLayoutView, "ÎπÑÎ?Î≤àÌò∏ ?ïÏù∏???§Î¶Ö?àÎã§.", Snackbar.LENGTH_LONG)
                    .show();
            return false;
        }else if(passwordEt.getText().length() < 8) {
            Snackbar
                    .make(coordinatorLayoutView, "ÎπÑÎ?Î≤àÌò∏Î•?8?êÎ¶¨ ?¥ÏÉÅ ?ÖÎ†•?¥Ï£º?∏Ïöî.", Snackbar.LENGTH_LONG)
                    .show();
            return false;
        }else if(nickname.getText().length() < 6){
            Snackbar
                    .make(coordinatorLayoutView, "?âÎÑ§?ÑÏùÑ 6?êÎ¶¨ ?¥ÏÉÅ ?ÖÎ†•?¥Ï£º?∏Ïöî.", Snackbar.LENGTH_LONG)
                    .show();
            return false;
        }
        return true;
...
//                        Snackbar.make(coordinatorLayoutView, json.get("error").toString(), Snackbar.LENGTH_LONG).show();
                        Snackbar
                                .make(coordinatorLayoutView, "?òÎ™ª???¥Î©î???êÎäî ?âÎÑ§???êÎäî ÎπÑÎ?Î≤àÌò∏?ÖÎãà??", Snackbar.LENGTH_LONG)
                                .show();
                    } else {
                        Toast.makeText(getApplicationContext(), "Í∞Ä?ÖÏù¥ ?ÑÎ£å?òÏóà?µÎãà??", Toast.LENGTH_SHORT);
                        finish();
                    }
